...
class SolverSession:
    ...
    def _capture_solver_details(self, response: dict) -> None:
        """Capture optional runtime facts without requiring them from Lumen."""
        profile = self.resolved.protocol_profile
        if profile is not None:
            self.diagnostics.solver_name = profile.display_name
        for key in ("solver_name", "solver_codename"):
            value = response.get(key)
            if isinstance(value, str) and value.strip():
                self.diagnostics.solver_name = value.strip()
                break
        backend = response.get("solver_backend")
        if isinstance(backend, str) and backend.strip():
            self.diagnostics.solver_backend = backend.strip()
        for key in ("solver_device", "device_name", "gpu_name"):
            value = response.get(key)
            if isinstance(value, str) and value.strip():
                self.diagnostics.solver_device = value.strip()
                break
        raw = response.get("solver_telemetry", response.get("telemetry", {}))
        if not isinstance(raw, dict):
            raw = {}
        aliases = {
            "frame_time": "FRAME_TIME", "step_time": "STEP_TIME",
            "matrix_assembly": "MATRIX_ASSEMBLY",
            "linear_solve": "LINEAR_SOLVE", "pcg_solve": "LINEAR_SOLVE",
            "line_search": "LINE_SEARCH", "step_advanced": "STEP_ADVANCED",
            "contact_memory": "CONTACT_MEMORY", "stretch": "STRETCH",
        }
        captured = {}
        for source, target in aliases.items():
            value = raw.get(source, response.get(source))
