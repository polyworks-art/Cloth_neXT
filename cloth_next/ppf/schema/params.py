...
"""PPF Param payload with the real Cloth NeXt material and Pin mapping.

All keys use the upstream kebab-case spellings. Present keys are forwarded to
the solver parameter table, while absent keys retain upstream defaults. A Pin
with positive ``pull_strength`` is a soft pull constraint; omitting that field
keeps the solver's exact hard-fix behavior.
"""
...
from ...materials.deformables import (
    RigidBodyMaterialSettings,
    RodMaterialSettings,
    SoftBodyMaterialSettings,
)
from ...materials.validation import (
    validate_shell_values,
...
def float32_wire(value: float) -> float:
    """Round to the exact float32 stored in the solver parameter table."""
...
def shell_wire_params(shell: ShellMaterialSettings) -> dict[str, object]:
    """Return the exact SHELL group parameter dictionary."""
    validate_shell_values(shell)
    # Shell shrink remains disabled until upstream PPF can combine changing
    # rest lengths with its strain limiter. Keep the setting readable for
    # compatibility while emitting a neutral factor.
    shrink_factor = 1.0
    ...
    return {
        ...
        "pressure": float32_wire(
            shell.inflate_pressure if shell.enable_inflate else 0.0),
        "stitch-stiffness": float32_wire(shell.sewing_stiffness),
    }


def static_wire_params(static: StaticMaterialSettings) -> dict[str, object]:
    """Return the exact STATIC group parameter dictionary."""
...
def rod_wire_params(rod: RodMaterialSettings) -> dict[str, object]:
    return {
        "model": "arap",
        "density": float32_wire(rod.linear_density),
...
def soft_body_wire_params(
        soft: SoftBodyMaterialSettings, uuid: str) -> dict[str, object]:
    tet = ({} if soft.tetrahedralizer == "ftetwild"
           else {uuid: {"backend": "tetgen"}})
    return {
        "model": "arap",
        "density": float32_wire(soft.volume_density),
...
def rigid_body_wire_params(
        rigid: RigidBodyMaterialSettings) -> dict[str, object]:
    """PDRD uses an exact rigid transform; only mass/contact are editable."""
    return {
        "model": "pdrd",
...
class SimulationSettings:
    """Immutable scene-level inputs for the solver payload."""
    ...
    def __post_init__(self) -> None:
        ...
        for key, entries in self.dynamic_parameters:
            if key not in _DYNAMIC_VECTOR_KEYS | _DYNAMIC_SCALAR_KEYS:
                raise ParamEncodeError(
                    f"unsupported dynamic parameter {key!r}")
            if key in seen:
                raise ParamEncodeError(
                    f"duplicate dynamic parameter {key!r}")
            seen.add(key)
            if len(entries) < 2:
                raise ParamEncodeError(
...
def _scene_wire_params(settings: SimulationSettings,
    ...
    for key, value in (
            ("air-density", settings.air_density),
...
def _attach_dynamic_params(
        payload: dict, settings: SimulationSettings) -> dict:
...
def _pin_wire_config(static_pin: StaticPinConfig,
                     index: int, offset: int) -> dict[str, object]:
    """Encode one exact-fix or soft-pull Pin entry.

    The solver treats a positive ``pull_strength`` as a soft force. Omitting the
    field preserves the exact Dirichlet hard-fix path; explicitly writing zero
    is avoided so the wire payload matches the official encoder's intent.
    """
    ...
    config: dict[str, object] = {
        "pin_group_id": static_pin.pin_group_id,
        "operations": [],
    }
    if strength > 0.0:
    ...
    if static_pin.times:
        ...
        config["pin_anim"] = {
            index: {
                "time": list(static_pin.times),
                "position": [
                    list(frame[offset]) for frame in static_pin.positions],
            }
        }
    return config
...
def build_param_payload(
    ...
    if static_pin is not None:
        pin_config[cloth_uuid] = {
            index: _pin_wire_config(static_pin, index, offset)
...
def build_multi_collider_param_payload(
    ...
    if not entries:
        return build_multi_deformable_param_payload(
            settings,
            ((cloth_name, cloth_uuid, "SHELL", shell, static_pin),),
            (),
...
def build_multi_deformable_param_payload(
    ...
    for name, uuid, group_type, material, static_pin in dynamic_entries:
        if not name.strip() or not uuid.strip():
            raise ParamEncodeError(
                "deformable name and uuid must not be empty")
        if uuid in seen:
            raise ParamEncodeError(f"duplicate deformable uuid {uuid!r}")
        seen.add(uuid)
        if (group_type == "SHELL"
                and isinstance(material, ShellMaterialSettings)):
            params = shell_wire_params(material)
        elif (group_type == "ROD"
              and isinstance(material, RodMaterialSettings)):
            params = rod_wire_params(material)
        elif (group_type == "SOLID"
              and isinstance(material, SoftBodyMaterialSettings)):
            params = soft_body_wire_params(material, uuid)
        elif (group_type == "PDRD"
              and isinstance(material, RigidBodyMaterialSettings)):
