...
_ROWS = (
    ...
    ("CNX-E101", "Scene validation", "No enabled deformable object", "Enable Cloth NeXt on at least one Cloth, Cable / Rope, Soft Body, or Rigid Body object."),
    ...
    ("CNX-E104", "Scene validation", "Invalid material, rest-shape, or solver-quality values", "Correct the highlighted value. Shrink cannot be combined with an enabled non-zero Stretch Limit."),
    ("CNX-E105", "Scene validation", "Invalid Pinning or constraint setup", "Check Pin Groups, Advanced Pin Motion targets, Soft Constraints, target topology, and overlapping Pin Groups."),
    ...
    ("CNX-E108", "Scene validation", "Invalid Force configuration", "Use a supported Force on an Empty and verify that its animated values and transform are finite."),
    ...
    ("CNX-E123", "Bake preparation", "Simulation scene or parameter encoding failed", "Check geometry, materials, Pins, Constraints, Forces, and finite numeric values."),
    ("CNX-E124", "Bake preparation", "Animated Pin target capture failed", "Check each animated Pin target, keep evaluated topology stable, and keep target objects available throughout the Bake range."),
    ...
    ("CNX-E128", "Bake preparation", "Animated Collider capture failed", "Keep the Collider's evaluated topology stable and check the reported frame, transform, and active simulation proxy."),
    ...
    ("CNX-E130", "Solver startup", "Unclassified solver startup failure", "Run the solver health check, then repair the selected managed installation or choose another compatible solver."),
    ...
    ("CNX-E162", "Simulation", "Intersection blocked the simulation from advancing", "Use Intersection Diagnostics and Auto Fix Intersections for mapped initial self-intersections. Otherwise separate the geometry; for inflation or self-contact, lower Pressure, add clearance, or use a smaller Time Step."),
    ...
    ("CNX-E165", "Simulation", "Non-finite simulation result", "Reduce Time Step and extreme Force, material, constraint, or Motion Override values; repair invalid input geometry."),
    ("CNX-E166", "Simulation", "RAM safety limit or solver memory exhaustion", "Lower scene complexity; raise the Auto-Cancel on High RAM threshold only when sufficient system memory is available."),
    ...
    ("CNX-E169", "Simulation", "Unexpected solver state", "Stop other solver work, run the solver health check, and start a fresh Bake."),
    ...
    ("CNX-E188", "Playback cache", "Cable / Rope Curve playback could not be applied", "Restore the original Cable / Rope Curve topology and remove conflicting Curve animation."),
...
_STAGE_RULES = {
    "PREPARING": _compile_rules((
        ...
        (r"\bpin(?:ning)?\b.*(?:topology|group|target)|"
         r"\banimated pin(?:ning)?\b|advanced pin motion|soft constraint",
         "CNX-E105"),
...
_RULES = _compile_rules((
    ...
    (r"material|quality|time step|substep|"
     r"shrink.*stretch limit|stretch limit.*shrink", "CNX-E104"),
