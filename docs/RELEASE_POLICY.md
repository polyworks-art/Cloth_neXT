...
One local Blender repository owns the installed Cloth NeXt package. Its module,
directory, Python namespace, preferences and installed files stay unchanged when
switching channels. Only its remote feed URL changes:

- `stable/` exposes one current Stable target.
- `beta/` exposes one current target from Stable or Beta releases.
- `dev/` exposes one current target from Stable, Beta, or Dev releases.

Stable publication updates Stable, Beta, and Dev; Beta publication updates Beta
and Dev; Dev publication updates Dev only. Users keep their selected feed while
receiving permitted release levels. This cumulative visibility was explicitly
confirmed by the owner for the 2.5.0 release. All three legacy feed paths
remain available for bridge delivery. Published archives remain immutable and
addressable, including archives retained from cumulative publications. Indexes
are generated by official Blender tooling using only the current target archive.

The bridge resolves `bl_ext.<repo-module>.cloth_next` to the owning RNA repository
and configures the selected feed on a deferred startup callback. It synchronizes
that same directory and validates its cached index before marking the session
migrated. Repeating startup or retrying after failure is safe. Other repository
entries, preferences, license data and installed package files are untouched.

Dev is public and experimental. Developer Tools are independent diagnostics;
entering Dev retains the explicit risk acknowledgement, while existing Dev users
can migrate automatically. All six channel switches are actionable even when
the selected target is lower. Same-channel updates require a newer target;
equal versions are up to date. Installation remains a handoff to Blender's
Extension Manager and never replaces the running package from Cloth NeXt code.

Dev publication remains confirmation-gated, uses an exact source commit SHA,
and creates no tag. This architecture change itself publishes nothing.
...
8. generates copies and official indices for every feed permitting this release
   level (Stable: all three; Beta: Beta and Dev);
