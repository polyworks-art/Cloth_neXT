...
- `stable/` exposes one Stable target.
- `beta/` exposes one Beta target.
- `dev/` exposes one Dev target.

Publication updates only the release's own channel. All three legacy feed paths
