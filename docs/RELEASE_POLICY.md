...
## 4. Customer and private distribution

Normal customer releases from 2.7.0 are distributed through Superhive using
Blender's native extension repositories. The add-on uses its owning repository,
never rewrites its URL, and exposes no Stable/Beta/Dev selector. It does not
implement authentication or embed the private publication destination.

Every private GitHub release with a numeric version greater than 2.6.0 uses one
shared repository, regardless of its Stable/Beta/Dev version counter. The full
index URL is configured exclusively in the GitHub Actions secret
`CLOTH_NEXT_RELEASE_REPOSITORY_URL`. Never write its value or directory identifier
into tracked source, documentation, test fixtures, onboarding content or add-on
packages. Workflows derive and mask the Pages directory at runtime. Missing or
invalid configuration fails closed; never fall back to a legacy channel.

The historical `stable/`, `beta/` and `dev/` repositories remain available.
2.6.0 was published to Beta and Dev; Stable remains unchanged. Beta is permanently
frozen at 2.6.0 as the customer migration bridge. Releases after 2.6.0 do not
modify any of these legacy feeds. This owner-approved unified private routing
supersedes the earlier requirement to continue publishing new builds to `dev/`.

Version counters, immutable tags for tagged releases, exact-SHA Dev dispatch,
all validation gates and canonical artifact rules remain in force. Distribution
destination is independent of the counter's historical release-level name.
...
15. exact channel target validation;
...
8. generates an official single-candidate index for the resolved publication
   destination (the configured private repository for every version after 2.6.0);
...
Dev versions after 2.6.0 publish only to the same configured private repository
as other release levels. They create no tag or GitHub Release and do not modify
legacy Stable, Beta or Dev. Earlier Dev versions use the historical Dev path.
...
- reports Superhive connection and cached purchase status in Preferences;
- uses the native owning repository for versions after 2.6.0, without embedding a private repository URL;
- synchronizes the native owning repository when explicitly requested;
...
- require explicit repository selector, version, and confirmation;
- use selector `release` and the configured secret for versions after 2.6.0; reject attempts to repair those versions into legacy feeds;
...
13. verify the canonical artifact and every affected channel repository,
    including inherited targets in Beta and Dev;
...
Keep the immutable 2.6.0 Beta archive and index available indefinitely. Its
migration notice directs customers to Superhive and existing installations keep
working. QuickAdd / Quick Assign is intentionally absent from 2.6.0; it first
ships in 2.7.0. The subsequent private publication rule in section 4 does not
rewrite the 2.6.0 artifact, tag, notice, or historical feed contents.
