...
- Add-on update check (Phase 2.8B): Blender 5.1.2 exposes public operators for
  ...
  `extensions.repo_sync`, `extensions.package_install`,
  ...
  Blender's own `repo_sync` + `package_install(repo_directory=, pkg_id=)` —
  the operator behind Blender's own per-package "Update" button — with
  `userpref_show_for_update` as the manual fallback.
